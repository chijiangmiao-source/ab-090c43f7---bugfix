...
- `let` 绑定在绑定处泛化为类型方案，**每次引用都重新实例化**（新鲜变量，互不影响）；
  但只量化「在类型中自由、却不在当前类型环境中自由」的变量——**局部宏捕获的外层读数
  （如外层函数参数 / 已声明传感器）其单位仍自由存在于环境中，绝不泛化**：
  同一局部宏的多次调用共享这一份量纲，长度与时间的相互矛盾约束会被稳定拒绝并定位两次调用；
...
3. **HTTP 复核**：恒等宏跨量纲复用（两份独立正确类型与输出类型）、
   异单位相加（定位两个操作数、不保留成功结论）、
   嵌套局部宏捕获外层读数（重复推断均失败且两次调用定位一致、不保留结论）、
   自应用（稳定的无限类型错误与位置）。
